...
class ExperimentBuilder(nn.Module):
    ...
    def run_inference_iter(self, x):
        ...
        logits_out = self.model.forward(x)  # forward the data in the model and return logits
        probility_distribution_out = F.softmax(logits_out, dim=1) # spits out probability distribution

        _, argmax_out = torch.max(logits_out.data, 1)  # get argmax of predictions
